...
def test_sequential_scheduler_with_dynamic_schedule_file_edit(dev_str):
    ...
    new_schedule_dict = dict(
        **{"minimal_again": ["demos.simple_example.main", ""]}, **original_schedule_dict
    )
    ...
    def write_to_file():
        with open(schedule_filepath, "w") as schedule_file:
            json.dump(
                new_schedule_dict, schedule_file, indent=2, separators=(",", ": ")
            )
    ...
    def run_scheduler(success_list):
        ...
        if num_completed_tasks == 3:
            ...
            raise Exception(
                "Expected 3 tasks to run, but actually "
                + str(num_completed_tasks)
                + " ran."
            )
